...
import { X, Clock, Users, Maximize2, ChevronLeft, ChevronRight, Images, Sparkles, ChefHat } from "lucide-react";
                        ...
                        <div className="flex-1 flex flex-col overflow-hidden bg-white dark:bg-slate-950">
                            <div className="flex-1 p-6 md:p-8 lg:p-10 flex flex-col overflow-y-auto custom-scrollbar pt-8">
                                ...
                                <motion.div
                                    ...
                                    className="text-slate-700 dark:text-slate-200 mb-6 text-sm sm:text-base italic border-l-4 border-bd-red pl-4 py-2 font-medium bg-rose-50/60 dark:bg-rose-950/20 rounded-r-2xl"
                                >
                                ...
                                <motion.div
                                    ...
                                    className="flex items-center gap-6 text-xs font-bold text-slate-600 dark:text-slate-300 mb-8 py-3.5 px-4 rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800 shadow-xs"
                                >
                                    <div className="flex items-center gap-2">
                                        <div className="w-8 h-8 rounded-xl bg-rose-100 dark:bg-rose-950/80 flex items-center justify-center text-bd-red">
                                            <Clock size={16} />
                                        </div>
                                        <div>
                                            <span className="text-[10px] uppercase font-bold text-slate-400 block leading-none">Prep Time</span>
                                            <span className="font-extrabold">{recipe.prepTime || "30 MIN"}</span>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-2 border-l border-slate-200 dark:border-slate-800 pl-6">
                                        <div className="w-8 h-8 rounded-xl bg-emerald-100 dark:bg-emerald-950/80 flex items-center justify-center text-bd-green">
                                            <Users size={16} />
                                        </div>
                                        <div>
                                            <span className="text-[10px] uppercase font-bold text-slate-400 block leading-none">Servings</span>
                                            <span className="font-extrabold">{recipe.serves || "4 PERSONS"}</span>
                                        </div>
                                    </div>
                                </motion.div>
                                ...
                                <div className="space-y-8 mb-6">
                                    ...
                                    >
                                        <h4 className="flex items-center gap-2.5 text-xs font-black text-bd-green dark:text-emerald-400 mb-4 uppercase tracking-wider">
                                            <span className="w-2 h-2 rounded-full bg-bd-green dark:bg-emerald-400" />
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                                            {recipe.ingredients.map((item, idx) => (
                                                <motion.div
                                                    ...
                                                    className="flex items-center gap-2.5 p-2.5 rounded-xl bg-slate-50 dark:bg-slate-900/70 border border-slate-100 dark:border-slate-800"
                                                >
                                                    <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 flex-shrink-0" />
                                                    <span className="text-slate-700 dark:text-slate-300 text-xs sm:text-sm font-medium leading-tight">
                                                        {item}
                                                    </span>
                                                </motion.div>
                                            ))}
                                    ...
                                    >
                                        <h4 className="flex items-center gap-2.5 text-xs font-black text-bd-red mb-4 uppercase tracking-wider">
                                            <ChefHat size={15} />
                                            Step-by-Step Method ({recipe.instructions.length} Steps)
                                        </h4>
                                        <div className="space-y-3">
                                            {recipe.instructions.map((step, idx) => (
                                                <motion.div
                                                    ...
                                                    className="flex gap-3.5 p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-900/70 border border-slate-100 dark:border-slate-800 group hover:border-bd-green/30 transition-colors"
                                                >
                                                    <span className="flex-none w-7 h-7 rounded-xl bg-bd-green text-white text-xs font-black flex items-center justify-center shadow-xs group-hover:bg-bd-red transition-colors">
                                                        {(idx + 1).toString().padStart(2, "0")}
                                                    </span>
                                                    <p className="text-slate-700 dark:text-slate-300 text-xs sm:text-sm leading-relaxed pt-0.5 font-normal">
                                                        {step}
                                                    </p>
                                                </motion.div>
                                            ))}
                                    ...
                                    >
                                        <div className="flex items-center justify-between mb-3">
                                            <h4 className="text-xs font-black uppercase tracking-wider text-bd-green dark:text-emerald-400 flex items-center gap-2">
                                                <Images size={15} /> Recipe Photos ({images.length})
                                            </h4>
